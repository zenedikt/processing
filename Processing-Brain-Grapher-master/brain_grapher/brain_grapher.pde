...
void poorSignalEvent(int sig) {
  channels[0].addDataPoint(sig);
  println("SignalEvent "+sig);
}

public void attentionEvent(int attentionLevel) {
  println("Attention Level: " + attentionLevel);
    channels[1].addDataPoint(attentionLevel);
}


void meditationEvent(int meditationLevel) {
  println("Meditation Level: " + meditationLevel);
  channels[2].addDataPoint(meditationLevel);
}


public void eegEvent(int delta, int theta, int low_alpha, int high_alpha, int low_beta, int high_beta, int low_gamma, int mid_gamma) {  
  println("eegEvent: "+delta);

  channels[3].addDataPoint(delta);
  channels[4].addDataPoint(theta);
  channels[5].addDataPoint(low_alpha);
  channels[6].addDataPoint(high_alpha);
  channels[7].addDataPoint(low_alpha);
  channels[8].addDataPoint(high_alpha);
  channels[9].addDataPoint(low_gamma);
  channels[10].addDataPoint(mid_gamma);  
}
